...
print(len(CPA_dict))
for record in SeqIO.parse('/home/mad149/Metagenome_grassmere/Retrieved_CPAseqs/retrieved_from_bins/CPA_sequences_vs_HQMQ_bins_filtered_fl.fasta', 'fasta'):
    if str(record.seq) not in CPA_dict:
        id_list = [record.id]
        CPA_dict[str(record.seq)] = id_list
...
for record in SeqIO.parse('/home/mad149/Metagenome_grassmere/Retrieved_CPAseqs/retrieved_from_contigs/CPA_sequences_from_contigs_prokka_translated_hmmaligned_filtered_fl.fasta', 'fasta'):
    if str(record.seq) not in CPA_dict:
        id_list = [record.id]
        CPA_dict[str(record.seq)] = id_list
